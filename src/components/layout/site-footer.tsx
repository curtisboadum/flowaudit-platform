...
function SiteFooter() {
  ...
  const footerLinks: Record<string, FooterColumnData> = {
    solutions: {
...
function FooterBrand({ t }: { t: Dict }) {
  return (
    <div className="sm:col-span-2 lg:col-span-1">
      <Link href="/" className="font-serif text-xl font-normal text-[#2F3037]">
        FlowAudit
      </Link>
      <p className="mt-3 max-w-[280px] font-sans text-sm leading-6 text-[#605A57]">{t.footer.tagline}</p>
      {/* Social icons */}
...
function FooterColumn({ column }: { column: FooterColumnData }) {
  return (
    <div>
      <h3 className="mb-4 font-sans text-sm font-semibold text-[#37322F]">{column.title}</h3>
